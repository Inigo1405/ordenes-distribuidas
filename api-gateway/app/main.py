...
async def get_current_user(
    ...
    try:
        async with httpx.AsyncClient(timeout=2.0) as client:
            r = await client.post(
                f"{AUTH_URL}/internal/verify", params={"token": token}
            )
...
async def get_order(order_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return {"order_id": order_id, "status": status, "data": order_data}
...
async def list_orders(current_user: dict = Depends(get_current_user)):
    ...
    for key in order_keys:
        order_id = (
            ...
            else key.decode().replace("order:", "")
        )
        order_data = redis_client.hgetall(key)
        owner_email = (
            order_data.get("owner_email")
            if isinstance(list(order_data.keys())[0] if order_data else "", str)
            else order_data.get(b"owner_email", b"").decode()
        )

        if is_admin or owner_email == user_email:
            status = redis_client.get(order_id)
            orders.append({"order_id": order_id, "status": status, "data": order_data})
...
async def auth_signup(data: SignupRequest):
    ...
    try:
        ...
        if r.status_code >= 400:
            return {
                "error": r.json().get("detail", "Error en auth service"),
...
async def auth_refresh(data: RefreshRequest):
    url = f"{AUTH_URL}/internal/refresh"
    try:
        async with httpx.AsyncClient(timeout=2.0) as client:
            r = await client.post(url, json=data.dict())
        r.raise_for_status()
