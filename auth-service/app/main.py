...
async def login(data: LoginRequest):
    try:
        ...
        async with Session() as session:
            ...
            redis_client.set(
                f"token:{jti}", "valid", ex=ACCESS_TOKEN_EXPIRE_MINUTES * 60
            )
...
def refresh_access_token(refresh_token: RefreshRequest):
    try:
        ...
        redis_client.set(
            f"token:{new_access_jti}", "valid", ex=ACCESS_TOKEN_EXPIRE_MINUTES * 60
        )
...
def hash_password(plain: str) -> str:
    salt = os.urandom(32)
    key = hashlib.pbkdf2_hmac("sha256", plain.encode("utf-8"), salt, iterations=260_000)
